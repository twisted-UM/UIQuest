...
public class SettingsScreen extends Screen {
    ...
    private boolean showEmptyChapters;
    private boolean hudEnabled;

    private boolean showChainInList;
    private boolean showChainInHud;
    ...
    private void loadValues() {
        ...
        showEmptyChapters = UIQuestConfig.SHOW_EMPTY_CHAPTERS.get();
        ...
        showChainInList = UIQuestConfig.SHOW_CHAIN_IN_LIST.get();
        showChainInHud  = UIQuestConfig.SHOW_CHAIN_IN_HUD.get();
        ...
        return switch (cat) {
            case GENERAL -> List.of(
                    ...
                    SettingEntry.slider("uiquest.settings.general.sound_volume",
                            ...
                            () -> soundVolume,
                            v -> soundVolume = v),
                    SettingEntry.button("uiquest.settings.general.reload_config",
                            () -> {
                                try {
                                    java.nio.file.Path configPath = net.neoforged.fml.loading.FMLPaths.CONFIGDIR.get()
                                            .resolve("uiquest-dev.toml");

                                    java.lang.reflect.Field field = net.neoforged.fml.config.ModConfig.class
                                            .getDeclaredField("loadedConfig");
                                    field.setAccessible(true);
                                    Object loadedConfig = field.get(UIQuest.devConfig);

                                    if (loadedConfig != null) {
                                        java.lang.reflect.Method configMethod = loadedConfig.getClass()
                                                .getDeclaredMethod("config");
                                        configMethod.setAccessible(true);
                                        com.electronwill.nightconfig.core.CommentedConfig config =
                                                (com.electronwill.nightconfig.core.CommentedConfig) configMethod.invoke(loadedConfig);
                                        ...
                                        UIQuestDevConfig.SPEC.afterReload();
                                        UIQuest.LOGGER.info("[UIQuest] Dev config reloaded");
                                    }
                                } catch (Exception e) {
                                    UIQuest.LOGGER.error("[UIQuest] Failed to reload dev config", e);
                                }
                            })
            );
            case GUI -> List.of(
                    ...
                    SettingEntry.toggle("uiquest.settings.gui.show_empty_chapters",
                            () -> showEmptyChapters,
                            v -> showEmptyChapters = v),
                    SettingEntry.toggle("uiquest.settings.gui.show_chain",
                            () -> showChainInList,
                            v -> showChainInList = v)
            );
            case HUD -> List.of(
                    ...
                    SettingEntry.toggle("uiquest.settings.hud.show_chain",
                            () -> showChainInHud,
                            v -> showChainInHud = v),
    ...
    private void renderEntry(GuiGraphics gfx, SettingEntry entry, int index,
        ...
        gfx.fill(bx, by, bx + bw, by + bh, hovered ? 0xFF555555 : 0xFF3A3A3A);
            String btnLabel = UIQuestLang.get(entry.labelKey() + ".btn");
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (x >= closeBtnX && x < closeBtnX + SAVE_BTN_W
                && y >= saveBtnY && y < saveBtnY + SAVE_BTN_H) {
            UISounds.play(UISounds.UI_CONFIG_CLICK);
            UIQuestConfig.SOUND_VOLUME.set(originalSoundVolume);
    ...
    private long lastPreviewSoundTime = 0;
    private static final long PREVIEW_SOUND_COOLDOWN = 100;
    ...
    private void updateSlider(int index, int mouseX) {
        ...
        if (entry.labelKey().equals("uiquest.settings.general.sound_volume")) {
            double oldVolume = soundVolume;
            entry.setDouble().accept(entry.min() + ratio * (entry.max() - entry.min()));
            UIQuestConfig.SOUND_VOLUME.set(soundVolume);

            if ((int)(oldVolume * 100) != (int)(soundVolume * 100)) {
                long now = System.currentTimeMillis();
                if (now - lastPreviewSoundTime >= PREVIEW_SOUND_COOLDOWN) {
                    UISounds.play(UISounds.UI_CLICK);
                    lastPreviewSoundTime = now;
                }
            }
        } else {
            entry.setDouble().accept(entry.min() + ratio * (entry.max() - entry.min()));
    ...
    private void saveAndClose() {
        ...
        UIQuestConfig.SHOW_EMPTY_CHAPTERS.set(showEmptyChapters);
        ...
        UIQuestConfig.SOUND_VOLUME.set(soundVolume);
        UIQuestConfig.SHOW_CHAIN_IN_LIST.set(showChainInList);
        UIQuestConfig.SHOW_CHAIN_IN_HUD.set(showChainInHud);
