...
public class QuestBrowserScreen extends Screen {
    ...
    private static final int COL_BG_OVERLAY     = 0xC0101010;
    ...
    private static final int TASK_ROW_H = TASKS_MAX_H / MAX_VIS_TASKS;
    ...
    private void buildGroups() {
        ...
        for (Chapter chapter : file.getAllChapters()) {
            ...
            if (g != null) {
                groupObjMap.putIfAbsent(g.getTitle().getString(), g);
                groupChapMap.computeIfAbsent(g.getTitle().getString(), k -> new ArrayList<>()).add(chapter);
        ...
        for (Map.Entry<String, ChapterGroup> e : groupObjMap.entrySet()) {
            ...
            for (Chapter ch : groupChapMap.getOrDefault(e.getKey(), List.of())) {
                List<TreeNode> questNodes = new ArrayList<>();
    ...
    private void renderSettingsButton(GuiGraphics gfx, int mouseX, int mouseY) {
        ...
        var rl = new net.minecraft.resources.ResourceLocation("uiquest", "textures/setting.png");
        gfx.blit(rl,
    ...
    private void renderDetailTasks(GuiGraphics gfx, Quest quest, TeamData td, int mouseX, int mouseY) {
        ...
        int TASK_ROW = dTasksH > 0 ? dTasksH / Math.min(total, MAX_VIS_TASKS) : TASK_ROW_H;
    ...
    private void renderDetailDesc(GuiGraphics gfx, Quest quest, int mouseX, int mouseY) {
        ...
        for (Component line : lines) {
            String contentsClass = line.getContents().getClass().getName();
            UIQuest.LOGGER.info("Line: class={}, text='{}', siblings={}",
                    contentsClass, line.getString(), line.getSiblings().size());
            if (!line.getSiblings().isEmpty()) {
                for (Component s : line.getSiblings()) {
                    UIQuest.LOGGER.info("  sibling: class={}, text='{}'",
                            s.getContents().getClass().getName(), s.getString());
                }
            }
            Object img = ImageComponentAccess.findIn(line);
            if (line.getContents().getClass().getName().equals("net.minecraft.network.chat.ComponentContents$1")) {
                UIQuest.LOGGER.info("Anonymous ComponentContents siblings: {}", line.getSiblings().size());
                for (Component s : line.getSiblings()) {
                    UIQuest.LOGGER.info("  sibling class: {}", s.getContents().getClass().getName());
                }
            }
            if (img != null) {
        ...
        renderScrollBar(gfx, sbX, dDescY + 2, dDescY + dDescH - 2,
                realTotalPixels + font.lineHeight * 2, dDescH, descScrollOffset);
    }
            ...
            if (x >= popX && x < popX + POPUP_W && y >= popY && y < popY + POPUP_H) {
                if (hoveredReward != null) {
                    if (hoveredReward instanceof ChoiceReward choiceReward) {
                        ...
                        new ClaimRewardMessage(hoveredReward.id, true).sendToServer();
                    }
                }
                return true;
            }
    ...
    public boolean mouseScrolled(double mx, double my, double delta) {
        ...
        if (entry != null && x >= listX && x < listX + LIST_W && y >= treeTop && y < listY + listH) {
            ...
            listScrollOffset = (int)Math.max(0, Math.min(
                    Math.max(0, entry.flat.size() - listVisibleRows), listScrollOffset - delta));
            return true;
        }
        if (selectedNode != null && selectedNode.type == NodeType.QUEST) {
            ...
            if (x >= detailX && x < detailX + detailW && y >= dDescY && y < dDescY + dDescH) {
                ...
                descScrollOffset = (int)Math.max(0, Math.min(
                        Math.max(0, totalPixels - dDescH + font.lineHeight * 2), descScrollOffset - delta * font.lineHeight));
                return true;
            }
        }
    ...
    public void renderBackground(GuiGraphics gfx) {
        gfx.fill(0, 0, width, height, 0xC8000000);
    }
    ...
    private void recalcDetailLayout(Quest quest) {
        int taskCount = Math.min(quest.getTasksAsList().size(), MAX_VIS_TASKS);
        dTasksH = taskCount > 0 ? taskCount * TASK_ROW_H : 0;
        dDescY  = dTasksY + dTasksH + (taskCount > 0 ? 12 : 4);
        subtitleH = font.lineHeight + 8;
        dDescY += subtitleH;
