...
public class QuestHudRenderer {
    ...
    public static void onRenderGuiOverlay(RenderGuiEvent.Post event) {
        ...
        if (allDone) {
            ...
            tasks.stream().filter(td::isCompleted).forEach(combined::add);
        ...
        QuestChainFinder.ChainInfo chain = QuestChainFinder.find(quest, td);
        int chainReserve = 0;
        if (chain.total() > 1 && UIQuestConfig.SHOW_CHAIN_IN_HUD.get()) {
            String chainStr = "[" + chain.position() + "/" + chain.totalStr() + "]";
            chainReserve = font.width(chainStr) + 4;
        }
        String questTitle = trimText(font, quest.getTitle().getString(), HUD_W - 12 - chainReserve);
        gfx.drawString(font, questTitle, textX, drawY, COL_TEXT, false);
        if (chain.total() > 1 && UIQuestConfig.SHOW_CHAIN_IN_HUD.get()) {
            String chainStr = "[" + chain.position() + "/" + chain.totalStr() + "]";
            int chainX = textX + font.width(questTitle) + 4;
            gfx.drawString(font, chainStr, chainX, drawY, COL_GOLD, false);
        }
